...
cmd.exe /c ""$UNREAL_ROOT/Engine/Build/BatchFiles/RunUAT.bat" -ScriptsForProject="$PROJECT_ROOT\$PROJECT_NAME.uproject" Turnkey -command=VerifySdk -platform=$platform -project="$PROJECT_ROOT/$PROJECT_NAME.uproject" BuildCookRun -nop4 -utf8output -nocompileeditor -skipbuildeditor -cook  -project="$PROJECT_ROOT/$PROJECT_NAME.uproject" -target=LyraServer  -unrealexe="$UNREAL_ROOT\Engine\Binaries\Win64\UnrealEditor-Cmd.exe" -platform=$platform -stage -archive  -package -build -pak -iostore -compressed -prereqs -archivedirectory="$OUTPUT_ROOT" -manifests -server -noclient -serverconfig=Development" -nocompile -nocompileuat